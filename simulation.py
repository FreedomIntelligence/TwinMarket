...
from util.UserDB import (
    get_all_user_ids, get_user_profile, build_graph, load_graph, 
    update_graph, save_graph, build_graph_new, get_top_n_users_by_degree
)
from util.ForumDB import (
    init_db_forum, execute_forum_actions, update_posts_score_by_date, 
    update_posts_score_by_date_range, create_post_db, get_all_users_posts_db
)
...
def process_user_input(
    user_id, user_db, forum_db, df_stock, current_date, debug, day_1st, 
    current_user_graph, import_news, df_strategy, is_trading_day, top_user, 
    log_dir, prob_of_technical, user_config_mapping, activate_maapping, 
    belief_args, config_path
):
    """
    处理单个用户的交易输入和决策过程
    ...
    try:
        ...
        is_random_trader = (
            user_strategy == "技术面" and 
            user_id not in top_user and 
            is_trading_day and 
            random.random() < prob_of_technical
        )
...
def init_simulation(
    ...
    while current_date <= end_date:
        ...
        print(f"用户关系图属性: {current_user_graph.number_of_nodes()} 个节点, {current_user_graph.number_of_edges()} 条边")
        
        # 根据节点度数获取顶级用户列表
        top_user = get_top_n_users_by_degree(
            G=current_user_graph, 
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            futures = [
                executor.submit(
                    process_user_input,
                    user_id, user_db, forum_db, df_stock, current_date, debug, 
                    day_1st, current_user_graph, import_news, df_strategy, 
                    is_trading_day, top_user, log_dir, prob_of_technical, 
                    user_config_mapping, activate_maapping, belief_args, 
            ...
            for future in tqdm(
                ...
                try:
                    # 等待任务完成，设置超时限制
                    user_id, forum_args, decision_result, post_response_args = future.result(
                    ...
                    # 超时处理：使用备用配置重试
                    print(f"[输入处理] 用户 {user_id} 超时: 处理超过 {TIMEOUT_THRESHOLD//60} 分钟。使用备用配置重试...")
                    fallback_config_path = './config/api.yaml'
                    retry_future = executor.submit(
                        process_user_input, user_id, user_db, forum_db, df_stock, 
                        current_date, debug, day_1st, current_user_graph, import_news, 
                        df_strategy, is_trading_day, top_user, log_dir, prob_of_technical, 
                        user_config_mapping, activate_maapping, belief_args, fallback_config_path
                    )
                    try:
                        # 重试任务的结果处理
                        user_id, forum_args, decision_result, post_response_args = retry_future.result(
                            timeout=TIMEOUT_THRESHOLD
                        )
                        forum_args_list.append((user_id, forum_args))
                        results.append((user_id, decision_result))
        ...
        reaction_result_file = os.path.join(reaction_result_dir, f"{current_date.strftime('%Y-%m-%d')}.json")
        ...
        if not day_1st:
                ...
                for user_id, forum_args in forum_args_list:
                    try:
                        ...
                        if isinstance(forum_args, str):
                            print(f"[论坛互动] 用户 {user_id} 的论坛参数错误: {forum_args}")
                            continue
            ...
            print(f"成功处理 {successful_actions}/{len(forum_args_list)} 个用户的论坛互动")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="初始化并运行股票交易模拟系统"
    )
    
    # ============================ 时间范围配置 ============================
    parser.add_argument(
        "--start_date", type=str, default="2023-06-15", 
        help="模拟开始日期 (格式: YYYY-MM-DD)"
    )
    parser.add_argument(
        "--end_date", type=str, default="2023-12-15", 
        help="模拟结束日期 (格式: YYYY-MM-DD)"
    )
    
    # ============================ 数据库配置 ============================
    parser.add_argument(
        "--forum_db", type=str, default="logs_100_0128_claude/forum_100.db", 
        help="论坛数据库文件路径"
    )
    parser.add_argument(
        "--user_db", type=str, default="logs_100_0128_claude/user_100.db", 
        help="用户数据库文件路径"
    )
    ...
    parser.add_argument(
        "--max_workers", type=int, default=50, 
        help="并发处理的最大线程数"
    )
    parser.add_argument(
        "--log_dir", type=str, default="logs_100_0128_claude", 
        help="日志文件保存目录"
    )
    
    # ============================ 用户关系图配置 ============================
    parser.add_argument(
        "--user_graph_save_name", type=str, default="user_graph_logs_100_0128_claude", 
        help="用户关系图保存文件名称"
    )
    parser.add_argument(
        "--similarity_threshold", type=float, default=0.2, 
        help="构建用户关系图的相似度阈值"
    )
    parser.add_argument(
        "--time_decay_factor", type=float, default=0.5, 
        help="构建用户关系图的时间衰减因子"
    )
    parser.add_argument(
        "--node", type=int, default=100, 
        help="用户关系图中的节点数量"
    )
    parser.add_argument(
        "--top_n_user", type=float, default=0.1, 
        help="顶级用户所占比例"
    )
    
    # ============================ 交易行为配置 ============================
    parser.add_argument(
        "--prob_of_technical", type=float, default=0.5, 
        help="技术面噪声交易者的激活概率"
    )
    parser.add_argument(
        "--activate_prob", type=float, default=1.0,
        help="用户激活参与模拟的概率"
    )
    
    # ============================ 数据文件配置 ============================
    parser.add_argument(
        "--belief_init_path", type=str, default="util/belief/belief_1000_0129.csv", 
        help="用户初始信念值文件路径"
    )
    parser.add_argument(
        "--config_path", type=str, default='./config/api.yaml', 
        help='API配置文件路径'
    )
