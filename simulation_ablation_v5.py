...
import os
import yaml
import trader.TradingAgentV4 as TradingAgent
...
import asyncio
import logging
from trader.broker_hs300_debug_zyf import test_matching_system, update_profiles_table_holiday
...
def process_user_input(user_id, user_db, forum_db, df_stock, current_date, debug, day_1st, current_user_graph, import_news, df_strategy, is_trading_day, top_user, log_dir, prob_of_technical, user_config_mapping, activate_maapping, belief_args, config_path):
    try:
        ...
        # 调用交易代理的输入信息方法
        user_id, decision_result, post_response_args, conversation_history = tradingAgent.input_info(
...
def init_simulation(
    ...
    user_db: str = 'data/UserDB/sys_100_hs300.db',
    debug: bool = True,
    max_workers: int = 1,
    ...
    # 读取全体重要新闻广播
    df_news = pd.read_pickle('data/update_long_news/sorted_impact_news_wmh.pkl')
    ...
    while current_date <= end_date:
        ...
        if is_trading_day:
            ...
            previous_trading_day = trading_days[current_index - 1]
            mask = (df_news['cal_date'] > previous_trading_day) & (df_news['cal_date'] <= current_date)
            import_news = sum(df_news[mask]['news'].tolist(), [])
        else:
            import_news = None
            ...
            for future in tqdm(as_completed(futures), total=len(all_user), desc=f"INPUT {current_date.strftime('%Y-%m-%d')}", unit="user"):
                try:
                    # Wait for the result with a timeout
                        user_id, decision_result, post_response_args = future.result(timeout=TIMEOUT_THRESHOLD)
                        if is_trading_day:  
                    ...
                    try:
                        user_id, decision_result, post_response_args = retry_future.result(timeout=TIMEOUT_THRESHOLD)
                        if is_trading_day:      
                            results.append((user_id, decision_result))
                            post_args_list.append((user_id, post_response_args))
...
def parse_args():
    ...
    parser.add_argument("--start_date", type=str, default="2023-01-04", help="Start date of the simulation (format: YYYY-MM-DD).")
    parser.add_argument("--end_date", type=str, default="2024-12-15", help="End date of the simulation (format: YYYY-MM-DD).")
    parser.add_argument("--forum_db", type=str, default="logs_100_0128_wmh/forum_100.db", help="Path to the forum database.")
    parser.add_argument("--user_db", type=str, default="logs_100_0128_wmh/user_100.db", help="Path to the user database.")
    parser.add_argument("--debug", type=bool, default=False, help="Enable debug mode.")
    parser.add_argument("--max_workers", type=int, default=20, help="Maximum number of worker threads for concurrent processing.")
    parser.add_argument("--user_graph_save_name", type=str, default="user_graph_logs_100_0128_wmh", help="Name of the user graph file.")
    ...
    parser.add_argument("--log_dir", type=str, default="logs_100_0128_wmh", help="Directory to save log files.")
