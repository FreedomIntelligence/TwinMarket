...
from util.ForumDB import init_db_forum, execute_forum_actions, update_posts_score_by_date, update_posts_score_by_date_range
import sqlite3
...
def process_user_input(user_id, user_db, forum_db, df_stock, current_date, debug, day_1st, current_user_graph, import_news, df_strategy, is_trading_day, top_user, log_dir, prob_of_technical, user_config_mapping):
    """
    处理单个用户的交易逻辑
    """
    # 创建日志目录
    ...
    try:
        ...
        # 执行交易逻辑
        user_id, forum_args = tradingAgent.input_info(
...
def process_user_output(user_id, tradingAgent, log_dir, current_date, user_config_mapping):
    log_dir = os.path.join(log_dir, current_date.strftime('%Y-%m-%d'))
    log_file = os.path.join(log_dir, f"{user_id}.log")
    logger = logging.getLogger(f"user_{user_id}")
    logger.setLevel(logging.INFO)
    ...
    try:
        user_id, decision_result = tradingAgent.output_decision()
        ...
        return user_id, decision_result
...
def init_simulation(
    ...
    while current_date <= end_date:
        ...
        config_list = ['./config_random/haiku.yaml',  # 0.3
                       ]
        config_prob = [1] # todo
        user_config_mapping = {}
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(
                    process_user_input, user_id, user_db, forum_db, df_stock, current_date, debug, day_1st, current_user_graph, import_news, df_strategy, is_trading_day, top_user, log_dir, prob_of_technical, user_config_mapping
                )
                for user_id in all_user
            ]
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(
                    process_user_output, user_id, tradingAgent, log_dir, current_date, user_config_mapping
                )
        ...
        if is_trading_day:
            test_matching_system(
                ...
                base_path='./',
                db_path=user_db,
                json_file_path=f"{log_dir}/{current_date.strftime('%Y-%m-%d')}.json"
            )
...
def parse_args():
    ...
    parser.add_argument("--forum_db", type=str, default="data/ForumDB/sample.db", help="Path to the forum database.")
    parser.add_argument("--user_db", type=str, default="data/UserDB/sample.db", help="Path to the user database.")
    parser.add_argument("--debug", type=bool, default=True, help="Enable debug mode.")
    parser.add_argument("--max_workers", type=int, default=3, help="Maximum number of worker threads for concurrent processing.")
    ...
    parser.add_argument("--node", type=int, default=3, help="Number of nodes in the user graph.")
