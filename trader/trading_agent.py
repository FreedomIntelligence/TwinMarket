...
class PersonalizedStockTrader:
    ...
    def input_info(
        ...
        self.system_context = TradingPrompt.get_system_prompt_new(
            self.user_profile, self.user_strategy, self.stock_profile_dict, self.stock_codes
        )

        # 构建用户初始提示和代理回复
        self.user_prompt = TradingPrompt.get_user_first_prompt(
            self.user_profile, self.user_strategy, self.stock_profile_dict, 
            pd.to_datetime(self.cur_date), self.is_trading_day, self.belief
        )
