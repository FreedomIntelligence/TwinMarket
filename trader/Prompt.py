...
class TradingPrompt:
    @staticmethod
    def get_system_prompt(user_profile: dict, user_strategy: str) -> dict:
        """生成系统提示词。
        ...
        for code, value in user_profile['cur_positions'].items():
            ...
            position_info = (
                f"- 持仓 {code}：{shares:,}股，持仓占比为{ratio}%,"
                f"持仓总市值{market_value:,}元，"
                f"昨天这只股票{'涨了' if yest_return_rate >= 0 else '跌了'}"
        ...
        return {
            "role": "system",
            "content": f"""你现在在进行角色扮演，请严格按照以下人设进行后续所有操作。
...
- 彩票型股票偏好：{user_profile['bh_lottery_preference_category']}（对高风险高收益股票的偏好程度）
- 投资集中度：{user_profile['bh_underdiversification_category']} （喜欢重仓某几只股票的程度）
...
"""
    ...
    def get_stock_summary(stock_code: str, stock_data: pd.Series) -> str:
        ...
        return f"""
    ## 股票代码：{stock_code} 上一个交易日{format_date(stock_data['date'])}的行情：
...
{{
    ...
    def get_query_for_na_prompt(user_type: str, stock_details: str, current_date: str) -> str:
        ...
        Returns:
            str: 格式化的提示词
        """
        return f"""根据历史交易情况和系统推荐，你目前所有关注的股票和相应行业如下：\n{stock_details}\n
今天是{current_date}，你正在查询与投资相关的新闻或公告来辅助你的投资。

根据你的投资偏好和当前市场情况，请思考以下问题：
1. 你目前最关心的行业或公司是什么？
2. 你希望获取哪些类型的信息？（例如：公司财报、市场趋势、政策变化、具体股票信息等）
3. 你是否有特定的关键词或主题需要进一步了解？
...
"""
    ...
    你的输出应该按照 YAML 格式：
    ```yaml
    queries:  # list[str]，必填，每个字符串代表一个独立的查询问题，问题应该是疑问句，比如<白酒消费趋势>而不是<白酒消费升级>
    - 你的问题1
    - 你的问题2
    stock_id:  # list[str]，选填，每个字符串代表一个想查询的公司对应的股票代码
    - 股票代码1
    - 股票代码2
    ```"""
    ...
    @staticmethod
    def get_stock_selection_prompt(current_stock_details: str, potential_stock_details: str, belief: str = None) -> str:
        """生成选股提示词。
        ...
        Returns:
            str: 格式化的选股提示词
        """
        return f"""结合你的人设和投资风格，从你目前持仓的股票和系统推荐的股票中选择所有潜在进行交易的股票:
## 你的持仓情况:
...
## 请以 YAML 格式返回你今天想要重点关注的股票列表和原因：
## 请注意：reason 字段应该包含你选择这些股票的原因，比如基于你的人设和投资风格，或者基于你的 belief，用一段话阐述
```yaml
selected_stocks:  # 选择所有你想要交易的股票，如果没有想交易的股票，返回空列表即可
- 股票代码1
- 股票代码2
reason: 
```"""
    ...
    def get_initial_prompt(formatted_date: str, stocks_to_deal: list, stock_summary: str,
        ...
        """
        return f"""
今天是 {formatted_date}，根据前面的对话，我们得知你认为有潜在交易可能的股票为：**{', '.join(stocks_to_deal)}**。在这个过程中，你只能获取到昨天以及之前的历史数据。
...
reason:
    ...
    def get_analysis_prompt(stocks_to_deal: list) -> str:
        ...
        """
        return f"""请根据你之前获得的所有信息，包括新闻、公告、市场、额外的股票数据，针对自己想要交易的股票：**{', '.join(stocks_to_deal)}** 进行分析。
...
#             str: 格式化的决策提示词
#         """
#         return f"""现在是做出最终交易决策的时候。请基于之前的分析，结合你的投资风格和人设，对每支股票做出具体的交易决策。
...
# - **重要：不投资的钱没有任何收益，但是投资有风险，也应当谨慎**
# - 确保每个交易决策都在价格和仓位限制范围内
# - 如果选择 hold，trading_position 应该等于 0
# - 如果某只股票当前仓位为 0，则意味着是系统推荐的股票，我建议你买入，当然你也可以选择持有
...
# SH688111:
#     action: sell
#     trading_position: 13.9
#     target_price: 24.0

# ## 请按照以下 YAML 格式输出你的决策：
# ```yaml
# Stock Code 1:
#     action: (buy/sell/hold)  # 交易动作，必须是 "buy"、"sell" 或 "hold" 之一
#     trading_position: (float)  # 计划交易的金额占总资产的比例，以百分比表示，必须是 0 到 100 之间的浮点数
#     target_price: (float)  # 目标交易价格，必须是跌停价和涨停价之间的浮点数
...
# ```"""
    ...
    {stock_id}:
        ...
        # 生成统一的决策提示词
        prompt = f"""现在是做出最终交易决策的时候。请基于之前的分析，结合你的投资风格和人设，对以下股票做出具体的交易决策。
    ...
    ## 交易规则：
    1. 交易价格必须在跌停价和涨停价之间
    2. 所有股票目标仓位之和不能超过剩余可用仓位

    ## 注意事项：
    - 所有决策必须符合你的投资风格和人设，但是你也应该根据当天的实际情况进行适当调整
    - 确保每个交易决策都在价格和仓位限制范围内
    - 如果选择 hold，target_position 应该等于当前仓位
    - 如果当前仓位为 0，则意味着是系统推荐的股票，我建议你买入，当然你也可以选择持有
    - 如果你选择 sell 或者 buy，请注意你的 target_price 的设置，如果设置的不合理，可能会导致交易失败，需要根据你的预期选择 target_price，而不是设置成昨日收盘价
    
    这是一个输出的例子：
    SH688111:
        action: buy
        cur_position: 10.5
        target_position: 18.5
        target_price: 24.0
    ...
    Stock Code 1:
        action: (buy/sell/hold)  # 交易动作，必须是 "buy"、"sell" 或 "hold" 之一
        cur_position: (float)  # 当前仓位，必须是 0 到 100 之间的浮点数
        target_position: (float)  # 目标仓位，必须是 0 到 100 之间的浮点数
        target_price: (float)  # 目标交易价格，必须是跌停价和涨停价之间的浮点数
...
"""
    ...
    def decision_json_to_prompt(decision_data: dict, recommendation_stocks: list) -> str:
        ...
        for stock_code, decision in stock_decisions.items():
            ...
            current_position = decision.get("current_position", "0")
