...
import re
from .utility import *
from .PromptV4 import TradingPrompt
from .StockRecV3 import StockRecommender
...
class PersonalizedStockTrader:
            ...
            if action != "hold":
                ...
                if target_price > 0:
                    ...
                    if quantity < 1:
                        ...
                        quantity = (quantity // 1) * 1   # 向下取整为1的倍数
    ...
    def input_info(
        ...
        # 全体新闻逻辑
            ...
            if self.is_trading_day:
                start_time = time.time()
                if not self.is_random_trader:
                    self.conversation_history.append({"role": "user", "content": self._give_all_info(self.stocks_to_deal)})
                    if len(self.stocks_to_deal) > 0:
                        # notice
        ...
        return self.user_id, self.decision_result, self.post_response_args, self.conversation_history
    ...
    def _generate_random_decision(self,) -> dict:
        # 获取前一天的股票数据
        df = self.df_stock.copy(deep=True)
    ...
    def _give_all_info(self, stock_list: list) -> str:
        """获取股票的详细技术指标信息
    
        Args:
            stock_list: 需要查询的股票列表
        ...
        info_str = "为了辅助你的投资决策，我将给你提供你潜在交易的指数前一交易日的详细数据。\n"
        for stock in stock_list:
            # 获取最近一个交易日的数据
            stock_data = df_stock[(df_stock['stock_id'] == stock) & 
                                (df_stock['date'] <= yesterday)].sort_values('date', ascending=False).iloc[0]
            
            info_str += f"\n## 指数 {stock} 前一交易日的详细指标:\n"
            
            # 基础价格信息
            info_str += f"- 收盘价: {stock_data['close_price']:.2f}\n"
            info_str += f"- 前一日收盘价: {stock_data['pre_close']:.2f}\n"
            info_str += f"- 涨跌额: {stock_data['change']:.2f}\n"
            info_str += f"- 涨跌幅: {stock_data['pct_chg']:.4f}%\n"
            
            # 成交量信息
            info_str += f"- 成交额: {stock_data['vol']:.0f}\n"
            info_str += f"- 5日平均成交额: {stock_data['vol_5']:.0f}\n"
            info_str += f"- 10日平均成交额: {stock_data['vol_10']:.0f}\n"
            info_str += f"- 30日平均成交额: {stock_data['vol_30']:.0f}\n"
            
            # 市场估值指标
            info_str += f"- 市盈率: {stock_data['pe_ttm']:.4f}\n"
            info_str += f"- 市净率: {stock_data['pb']:.4f}\n"
            # info_str += f"- 市销率: {stock_data['ps_ttm']:.4f}\n"
            # info_str += f"- 股息率: {stock_data['dv_ttm']:.4f}\n"
            
            # 均线指标
            info_str += f"- 5日移动均线价格: {stock_data['ma_hfq_5']:.3f}\n"
            info_str += f"- 10日移动均线价格: {stock_data['ma_hfq_10']:.3f}\n"
            info_str += f"- 30日移动均线价格: {stock_data['ma_hfq_30']:.3f}\n"
            
            # 资金流向
            info_str += f"- 超大单净资金流入: {stock_data['elg_amount_net']/10000:.2f}万元\n"
    ...
    def _get_user_indicators(self):
        ...
        n = random.randint(2, len(MAPPING_INDICATORS3[type]))
        selected_type_indicators = random.sample(MAPPING_INDICATORS3[type], n)
    ...
    def _make_final_decision(self, debug: bool = False) -> dict:
        ...
        # 决策--生成决策
        decision_prompt, yaml_template = TradingPrompt.get_decision_prompt(self.stocks_to_deal, self.price_info, position_info, available_position)
    ...
    def _polish_decision(self, decision_args: dict, cur_positions: dict, available_position: float) -> dict:
        '''
        做这样的事情：
        ...
        for stock_code, decision in stock_decisions.items():
            ...
            if new_decision["action"] == "hold" or trading_position == 0:
                ...
                new_decision["target_price"] = self.price_info.get(stock_code, {}).get('pre_close', 0)
            ...
            if stock_code in self.price_info:
                ...
                if new_decision["action"] != "hold":
                    ...
                    # 确保价格在涨跌停区间内
                    new_decision["target_price"] = round(min(max(random_price, limit_down), limit_up), 2)
                else:
