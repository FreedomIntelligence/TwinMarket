...
import re
from .utility import *
from .PromptV3 import TradingPrompt
...
class PersonalizedStockTrader:
    def __init__(self,
        ...
        self.system_context = TradingPrompt.get_system_prompt(user_profile, self.user_strategy,self.stock_profile_dict)
    ...
    def _get_rec_stock(self):
        rec_stock = STOCK_REC.recommend_portfolio(input_portfolio=self.stock_codes, top_n=2)
    ...
    def _get_stock_details(self, stock_list: list, type: str = "basic") -> str:
        ...
        for stock in stock_list:
            ...
            if not stock_info.empty:
                if type == "basic":
                    stock_details_str += f"- 股票代码：{stock}，名称：{stock_info['name'].iloc[0]}，行业：{stock_info['industry'].iloc[0]}，其包含的成分股为：{self.stock_profile_dict[stock]}\n"
                elif type == "full":
                    if stock in self.user_profile['cur_positions']:
                        ...
                        stock_details_str += f"- 股票代码：{stock},名称：{stock_info['name'].iloc[0]},行业：{stock_info['industry'].iloc[0]};持仓{shares:,}股，持仓占比为{ratio}%,持仓总市值{market_value:,}元，上个交易日这只股票{'涨了' if yest_return_rate >= 0 else '跌了'}{abs(yest_return_rate)}%，它总共让你{'赚了' if total_profit_rate >= 0 else '亏了'}{abs(total_profit_rate)}%，其包含的成分股为：{self.stock_profile_dict[stock]}\n"
                    else:
                        stock_details_str += f"- 股票代码：{stock}，名称：{stock_info['name'].iloc[0]}，行业：{stock_info['industry'].iloc[0]}，没有任何持仓信息，属于系统推荐股票，其包含的成分股为：{self.stock_profile_dict[stock]}\n"
            else:
